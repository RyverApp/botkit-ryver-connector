...
module.exports = function (Botkit, config) {
    ...
    controller.middleware.normalize.use(function normalizeMessageChannel(bot, message, next) {
        ...
        if (message.type.startsWith('chat_')) {
            switch (message.data.channel.__metadata.type) {
                case 'Entity.Forum':
                    channel = utils.formatChannel(utils.channelPrefix.FORUM, message.data.channel.id);
                    break;
                case 'Entity.Workroom':
                    channel = utils.formatChannel(utils.channelPrefix.WORKROOM, message.data.channel.id);
                    break;
                case 'Entity.User':
                    channel = utils.formatChannel(utils.channelPrefix.USER, message.data.channel.id);
                    break;
            }
